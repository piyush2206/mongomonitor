...
function monitor(config) {
    ...
    snoozeDuration *= config.snoozeInterval
}
...
monitor.prototype.verifyMemberHealthy = function* (db, host) {
    ...
    // console.log(`last :${rsLastMemberCount} | now :${rs.members.length}`)
    // for(let m of rs.members){
    //     console.log("member host:", m.name);
    // }
    // if (rsLastMemberCount !== undefined || rsLastMemberCount === rs.members.length) {
    //     return
    // }

    rsLastMemberCount = rs.members.length
    
    // Run 'rs.config()' on the node
    var config = yield db.executeDbAdminCommand({ replSetGetConfig: 1 });
    
    // Get a count of the votes available
    var votes = config.config.members.reduce(function reduceVotes(acc, member) {
      return acc + member.votes;  
    }, 0);
    ...
    if (votes % 2 === 0) {
        ...
        logger.notifyError(new Error(`Replica set configuration on host ${host} contains an even number of votes (${votes}) across ${rs.members.length} members which will cause primary elections to fail.`), this.config);
    }
...
monitor.prototype.verifySufficientOplogLength = function* (db, host) {
    ...
    if (data.length === 0) {
        !this.snooze('a10') &&
        logger.notifyError(new Error(`Failed to retrieve oldest oplog timestamp for host ${host}.`), this.config);
        return;
    }
...
monitor.prototype.snooze = function (alertId) {
    if (snoozeAlerts[alertId] && (new Date) - snoozeAlerts[alertId].timestamp < snoozeDuration){
        return true
    }

    snoozeAlerts[alertId] = {
        timestamp: new Date()
    }
    return false
}
